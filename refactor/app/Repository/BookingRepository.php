...
use App\Enums\TranslatorTypesEnum;
/**
    ...
    public function getUsersJobs($user_id)
{
    ...
    $usertype = $cuser->is(UserTypesEnum::Customer) ? UserTypesEnum::Customer : UserTypesEnum::Translator;
    ...
    if ($usertype === UserTypesEnum::Customer) {
        $jobs = $cuser->jobs()
            ...
            ->orderBy('due')
            ->get();
    } elseif ($usertype === UserTypesEnum::Translator) {
    ...
    foreach ($jobs as $job) {
        if ($job->immediate === 'yes') {
            ...
            $item['usercheck'] = Job::checkParticularJob($user_id, $job);
            $normalJobs[] = $item;
        }
    }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($usertype === UserTypesEnum::Customer) {
            ...
            $numpages = ceil($totaljobs / $recordPerPage);
            $jobs = $normalJobs = $jobs_ids;
        }
    
        return compact('emergencyJobs', 'normalJobs', 'jobs', 'cuser', 'usertype', 'numpages', 'pagenum');
    }
    ...
    {
        $response['status'] = $this->failedStatus;
        $immediatetime = 5;
        $consumer_type = $user?->userMeta?->consumer_type;
        //not get .env variable directly add into config
        if ($user?->user_type == config('customer_role_id')) {
           ...
           $data['customer_physical_type'] = $response['customer_physical_type'] =  isset($data['customer_physical_type']) ? 'yes' : 'no';
            ...
            if ($data['immediate'] == 'yes') {
                ...
                $data['immediate'] = $data['customer_phone_type']  = 'yes';
            ...
            $data['gender'] = match(true) {
                in_array('male', $data['job_for']) => 'male',
                in_array('female', $data['job_for']) => 'female',
                default => null,
            };
            $data['certified'] = match (true) {
                ...
                in_array('certified_in_health', $data['job_for']) => 'health',
                default => null,
            };
            $data['certified'] = match (true) {
                ...
                in_array('normal', $data['job_for']) && in_array('certified_in_health', $data['job_for']) => 'n_health',
                default => null,
            };
            ...
            $data['by_admin'] = isset($data['by_admin']) ?? 'no';
    ...
    public function storeJobEmail($data)
    {
        ...
        $job->user_email = data_get($data, 'user_email');
        //data_get is laravel method
        $job->reference = data_get($data,'reference','');
        $user = $job->user()->get()->first();
        if (isset($data['address'])) {
            $job->address = !empty($data['address']) ??  $user?->userMeta?->address;
            $job->instructions = !empty($data['instructions']) ?? $user?->userMeta?->instructions;
            $job->town = !empty($data['town']) ?? $user?->userMeta?->city;
        }
        $job->save();
    ...
    public function jobToData($job)
    {
        ...
        $dueDate = Carbon::parse($job->due)->toDateString();
        $dueTime = Carbon::parse($job->due)->toTimeString();
    ...
    public function jobEnd($post_data = array())
    {
        ...
        [$hours, $minutes] = explode(':', $job->session_time);
    ...
    {
        $user_meta = UserMeta::whereUserId($user_id)->first();
        $translator_type = $user_meta?->translator_type;
        $job_type = match($translator_type){
            TranslatorTypesEnum::Professional=>'paid',
            TranslatorTypesEnum::Rwstranslator=>'rws',
            TranslatorTypesEnum::Volunteer=>'unpaid',
            default=>'unpaid'
        };
    
        $languages = UserLanguages::whereUserId($user_id)->pluck('lang_id')->all();
    ...
    {
        $users = User::where(['user_type'=>2,'status'=>1])->whereNot('id',$exclude_user_id)->all();
        $translator_array = $delpay_translator_array = array(); 
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $jobPosterMeta = UserMeta::whereUserId($job->user_id)->first();

        // prepare message templates
        $date = Carbon::parse($job->due)->format('d.m.Y');
        $time = Carbon::parse($job->due)->format('H:i');
        $duration = Carbon::now()->addMinutes($job->duration)->diffInHoursAndMinutes(Carbon::now());
        $jobId = $job->id;
        ...
        $message = match ($job->customer_physical_type) {
            'yes' => match ($job->customer_phone_type) {
                'no' => $physicalJobMessageTemplate,
                'yes' => $phoneJobMessageTemplate,
            },
            default => $phoneJobMessageTemplate,
        };
        Log::info($message);
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $blacklist = UsersBlacklist::whereUserId($job?->user_id)->pluck('translator_id')->get();
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        ...
        $user = $job->user()->first();
        $email = !empty($job?->user_email) ?? $user?->email;
        $name = $user?->name;
        $dataEmail = [
            'user' => $user,
            'job'  => $job
...
//        }
     ...
     * @return bool
     */
    private function sendSessionEmail($user, $job, $session_time, $for_text)
    {
        $email = $user->email;
        $name = $user->name;
        $subject = 'Information om avslutad tolkning för bokningsnummer #' . $job->id;
    ...
    private function changeStartedStatus($job, $data)
    {
        ...
        if ($data['status'] === 'completed') {
            ...
            $this->sendSessionEmail($user, $job, $session_time, 'faktura');
    
            $translatorUser = $job->translatorJobRel->where(['completed_at' => null, 'cancel_at' => null])->first();
    
            if ($translatorUser) {
                $this->sendSessionEmail($translatorUser->user, $job, $session_time, 'lön');
            }
        }
...
//        }
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        $msg_text = $job->customer_physical_type == 'yes' ? array(
    ...
    private function changeAssignedStatus($job, $data)
    {
        if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24'])) {
                ...
                $user = $job->translatorJobRel->where(['completed_at'=>Null,'cancel_at'=>Null])->first();
    ...
    private function changeTranslator($currentTranslator, $data, $job)
    {
        ...
        if (!empty($data['translator_email'])) {
            $data['translator'] = User::whereEmail($data['translator_email'])->value('id');
        }
    
        if (!is_null($currentTranslator)) {
            if ((isset($data['translator']) && $currentTranslator->user_id != $data['translator']) || !empty($data['translator_email'])) {
                $newTranslatorData = array_except($currentTranslator->toArray(), 'id');
                $newTranslatorData['user_id'] = $data['translator'];
                $newTranslator = Translator::create($newTranslatorData);
                $currentTranslator->update(['cancel_at' => Carbon::now()]);
                $logData[] = [
        ...
        } elseif (isset($data['translator']) && $data['translator'] != 0) {
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        if ($cuser->is('customer')) {
            ...
            $withdrawHoursDiff = $job->withdraw_at->diffInHours($job->due);
    
            if ($withdrawHoursDiff >= 24) {
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        $languages = UserLanguages::where('user_id', $cuser->id)->pluck('lang_id')->toArray();
        ...
        foreach ($job_ids as $k => $job) {
            ...
            if (($job->customer_phone_type === 'no' || empty($job->customer_phone_type)) && $job->customer_physical_type === 'yes' && !$checktown) {
                unset($job_ids[$k]);
            }
        }
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            // ... Similar logic for the user case ...
    ...
    public function alerts()
    {
        ...
        $jobIds = $sesJobs->pluck('id')->toArray();
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestData['filter_timetype']) && in_array($requestData['filter_timetype'], ['created', 'due'])) {
                $fromColumn = $requestData['filter_timetype'] == 'created' ? 'created_at' : 'due';
                if (isset($requestData['from']) && $requestData['from'] != "") {
                    $allJobs->where("jobs.$fromColumn", '>=', $requestData['from']);
                }
                if (isset($requestData['to']) && $requestData['to'] != "") {
                    $to = $requestData['to'] . ' 23:59:59';
                    $allJobs->where("jobs.$fromColumn", '<=', $to);
                }
                $allJobs->orderBy("jobs.$fromColumn", 'desc');
            }
    ...
    public function reopen($request)
    {
        ...
        if ($job['status'] != 'timedout') {
            ...
            $job->update([
                ...
                'admin_comments' => "This booking is a reopening of booking # $job->id",
            ]);
        }
        
        // updating using relationship
